...
В текущей реализации outbox сохраняется в БД, 
но публикация события выполняется напрямую после commit. 
Отдельный relay-worker сознательно не реализован в рамках тестового
